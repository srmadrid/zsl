    ...
    if (comptime types.layoutOf(Y) == .col_major) {
        ...
        while (j < y.cols) : (j += 1) {
            ...
            while (p < y.ptr[j + 1]) : (p += 1) {
                ...
                if (y.idx[p] != j) {
                    op_(&o.data[o._index(j, y.idx[p])], x.data[x._index(j, y.idx[p])], numeric.conj(y.data[p]));
                }
            }
        }
    } else {
        ...
        while (i < y.rows) : (i += 1) {
            ...
            while (p < y.ptr[i + 1]) : (p += 1) {
                ...
                if (i != y.idx[p]) {
                    op_(&o.data[o._index(y.idx[p], i)], x.data[x._index(y.idx[p], i)], numeric.conj(y.data[p]));
                }
            }
        }
    }
