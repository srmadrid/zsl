            ...
            while (p < y.ptr[j + 1]) : (p += 1) {
                ...
                if (y.idx[p] != j) {
                    const tx2 = if (j < y.idx[p])
                        (if (comptime types.uploOf(X) == .upper) x.data[x._index(j, y.idx[p])] else numeric.zero(types.Numeric(X)))
                    else
                        (if (comptime types.uploOf(X) == .lower) x.data[x._index(j, y.idx[p])] else numeric.zero(types.Numeric(X)));

                    op_(&o.data[o._index(j, y.idx[p])], tx2, numeric.conj(y.data[p]));
                }
            }
        }
    } else {
            ...
            while (p < y.ptr[i + 1]) : (p += 1) {
                ...
                if (i != y.idx[p]) {
                    const tx2 = if (i == y.idx[p])
                        ...
                        (if (comptime types.uploOf(X) == .upper) x.data[x._index(y.idx[p], i)] else numeric.zero(types.Numeric(X)))
                    else
                        (if (comptime types.uploOf(X) == .lower) x.data[x._index(y.idx[p], i)] else numeric.zero(types.Numeric(X)));

                    op_(&o.data[o._index(y.idx[p], i)], tx2, numeric.conj(y.data[p]));
                }
            }
        }
    }
