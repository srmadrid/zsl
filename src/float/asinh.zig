...
pub fn asinh(x: anytype) @TypeOf(x) {
    ...
    switch (X) {
        f16 => return numeric.cast(f16, asinh32(numeric.cast(f32, x))),
        f32 => {
            // https://github.com/JuliaMath/openlibm/blob/master/src/s_asinhf.c
            return asinh32(numeric.cast(f32, x));
        },
        f64 => {
            // https://github.com/JuliaMath/openlibm/blob/master/src/s_asinh.c
            return asinh64(numeric.cast(f64, x));
        },
        f80 => {
            //
            // return asinh80(numeric.cast(f80, x));
            return numeric.cast(f80, asinh128(numeric.cast(f128, x)));
        },
        f128 => {
            // https://github.com/JuliaMath/openlibm/blob/master/ld128/s_asinhl.c
            return asinh128(numeric.cast(f128, x));
        },
        else => unreachable,
    }
}
