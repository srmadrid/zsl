...
fn getLogicalValue(mat: anytype, r: usize, c: usize, comptime skip_primary_search: bool) meta.Numeric(@TypeOf(mat)) {
    const M = @TypeOf(mat);
    ...
    if (comptime meta.isSymmetricMatrix(M) or meta.isHermitianMatrix(M)) {
        if (utils.searchSparse(mat, c, r)) |v| {
            return if (comptime meta.isHermitianMatrix(M)) numeric.conj(v) else v;
        }
    }

    if (r == c and comptime meta.diagOf(M) == .unit)
        return numeric.one(meta.Numeric(M));

    return numeric.zero(meta.Numeric(M));
}
...
fn processCoordinate(o: anytype, x: anytype, y: anytype, i_o: usize, j_o: usize, val_x: anytype, val_y: anytype, comptime op_: anytype) void {
    ...
    if (comptime meta.isSparseMatrix(O)) {
        var val_o: meta.Numeric(O) = undefined;
    ...
    if (comptime meta.isSymmetricMatrix(X) or meta.isHermitianMatrix(X) or
        meta.isSymmetricMatrix(Y) or meta.isHermitianMatrix(Y))
    {
        if (i_o != j_o) {
            const x_hits_naturally = utils.searchSparse(x, j_o, i_o) != null;
            var y_hits_naturally = utils.searchSparse(y, j_o, i_o) != null;

            if ((comptime meta.layoutOf(X) != meta.layoutOf(Y)) and
                (comptime meta.isSymmetricMatrix(X) or meta.isHermitianMatrix(X)) and
                (comptime meta.isSymmetricMatrix(Y) or meta.isHermitianMatrix(Y)) and
            ...
            if (!x_hits_naturally and !y_hits_naturally) {
                const mirror_x = if (comptime meta.isSymmetricMatrix(X)) val_x else if (comptime meta.isHermitianMatrix(X)) numeric.conj(val_x) else numeric.zero(meta.Numeric(X));
                const mirror_y = if (comptime meta.isSymmetricMatrix(Y)) val_y else if (comptime meta.isHermitianMatrix(Y)) numeric.conj(val_y) else numeric.zero(meta.Numeric(Y));
...
pub fn apply2_(o: anytype, x: anytype, y: anytype, comptime op_: anytype) !void {
    ...
    if (comptime meta.isSparseMatrix(O)) {
        const nnz =
            x.nnz * (if (comptime meta.isSymmetricMatrix(X) or meta.isHermitianMatrix(X)) 2 else 1) +
            y.nnz * (if (comptime meta.isSymmetricMatrix(Y) or meta.isHermitianMatrix(Y)) 2 else 1);
    ...
    if (comptime meta.layoutOf(X) == meta.layoutOf(Y)) {
        var outer: usize = 0;
        const limit = if (comptime meta.layoutOf(X) == .col_major) o.cols else o.rows;
        ...
        while (outer < limit) : (outer += 1) {
            ...
            while (px < x.ptr[outer + 1] and py < y.ptr[outer + 1]) {
                ...
                if (x.idx[px] == y.idx[py]) {
                    i_o = if (comptime meta.layoutOf(X) == .col_major) x.idx[px] else outer;
                    j_o = if (comptime meta.layoutOf(X) == .col_major) outer else x.idx[px];
                    ...
                    i_o = if (comptime meta.layoutOf(Y) == .col_major) y.idx[py] else outer;
                    j_o = if (comptime meta.layoutOf(Y) == .col_major) outer else y.idx[py];
