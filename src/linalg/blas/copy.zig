...
pub fn copy(
    ...
    if (comptime options.link_cblas != null and X == Y) {
        switch (comptime meta.numericType(X)) {
            .float => {
                if (comptime X == f32)
                    return linalg.cblas.scopy(n, x, incx, y, incy)
                else if (comptime X == f64)
                    return linalg.cblas.dcopy(n, x, incx, y, incy);
            },
            .complex => {
                if (comptime meta.Scalar(X) == f32)
                    return linalg.cblas.ccopy(n, x, incx, y, incy)
                else if (comptime meta.Scalar(X) == f64)
                    return linalg.cblas.zcopy(n, x, incx, y, incy);
            },
            else => {},
        }
    }
...
fn k_copy(n: isize, x: anytype, incx: isize, y: anytype, incy: isize) void {
