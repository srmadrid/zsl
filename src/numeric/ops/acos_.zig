...
/// ## Signature
/// ```zig
/// numeric.acos_(o: *O, x: X, ctx: anytype) !void
/// ```
///
/// ## Arguments
...
/// * `fn zmlAcos_(*O, X, anytype) !void`: Computes the arccosine of `x` and
...
pub inline fn acos_(o: anytype, x: anytype, ctx: anytype) !void {
    ...
    if (comptime types.isCustomType(O)) {
        if (comptime types.isCustomType(X)) { // O and X both custom
            ...
            if (comptime Impl != null) {
                return Impl.?.zmlAcos_(o, x, ctx);
            } else {
                var acos = try numeric.acos(x, ctx);
                defer numeric.deinit(&acos, ctx);
            ...
            if (comptime types.hasMethod(O, "zmlAcos_", fn (*O, X, anytype) anyerror!void, &.{ *O, X, @TypeOf(ctx) })) {
                return O.zmlAcos_(o, x, ctx);
            } else {
                var acos = try numeric.acos(x, ctx);
                defer numeric.deinit(&acos, ctx);
        ...
        if (comptime types.hasMethod(X, "zmlAcos_", fn (*O, X, anytype) anyerror!void, &.{ *O, X, @TypeOf(ctx) })) {
            return X.zmlAcos_(o, x, ctx);
        } else {
            var acos = try numeric.acos(x, ctx);
            defer numeric.deinit(&acos, ctx);
    ...
    switch (comptime types.numericType(O)) {
        .bool, .int, .float, .dyadic, .cfloat => switch (comptime types.numericType(X)) {
            ...
            .dyadic => @compileError("zml.numeric.acos_: not implemented for " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " yet."),
            .cfloat => {
                comptime types.validateContext(@TypeOf(ctx), .{});
            ...
            .integer => @compileError("zml.numeric.acos_: not implemented for " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " yet."),
            .rational => @compileError("zml.numeric.acos_: not implemented for " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " yet."),
            .real => @compileError("zml.numeric.acos_: not implemented for " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " yet."),
            .complex => @compileError("zml.numeric.acos_: not implemented for " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " yet."),
            .custom => unreachable,
        },
