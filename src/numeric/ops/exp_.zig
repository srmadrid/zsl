...
/// ## Signature
/// ```zig
/// numeric.exp_(o: *O, x: X, ctx: anytype) !void
/// ```
///
/// ## Arguments
...
/// * `fn zmlExp_(*O, X, anytype) !void`: Computes the exponential of `x` and
...
pub inline fn exp_(o: anytype, x: anytype, ctx: anytype) !void {
    ...
    if (comptime types.isCustomType(O)) {
        if (comptime types.isCustomType(X)) { // O and X both custom
            ...
            if (comptime Impl != null) {
                return Impl.?.zmlExp_(o, x, ctx);
            } else {
                var exp = try numeric.exp(x, ctx);
                defer numeric.deinit(&exp, ctx);
            ...
            if (comptime types.hasMethod(O, "zmlExp_", fn (*O, X, anytype) anyerror!void, &.{ *O, X, @TypeOf(ctx) })) {
                return O.zmlExp_(o, x, ctx);
            } else {
                var exp = try numeric.exp(x, ctx);
                defer numeric.deinit(&exp, ctx);
        ...
        if (comptime types.hasMethod(X, "zmlExp_", fn (*O, X, anytype) anyerror!void, &.{ *O, X, @TypeOf(ctx) })) {
            return X.zmlExp_(o, x, ctx);
        } else {
            var exp = try numeric.exp(x, ctx);
            defer numeric.deinit(&exp, ctx);
    ...
    switch (comptime types.numericType(O)) {
        .bool, .int, .float, .dyadic, .cfloat => switch (comptime types.numericType(X)) {
            ...
            .dyadic => @compileError("zml.numeric.exp_: not implemented for " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " yet."),
            .cfloat => {
                comptime types.validateContext(@TypeOf(ctx), .{});
            ...
            .integer => @compileError("zml.numeric.exp_: not implemented for " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " yet."),
            .rational => @compileError("zml.numeric.exp_: not implemented for " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " yet."),
            .real => @compileError("zml.numeric.exp_: not implemented for " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " yet."),
            .complex => @compileError("zml.numeric.exp_: not implemented for " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " yet."),
            .custom => unreachable,
        },
