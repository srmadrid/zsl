    ...
    if (comptime meta.layoutOf(X) == meta.layoutOf(Y)) {
        ...
        while (outer < x.rows) : (outer += 1) {
            ...
            while (px < x.ptr[outer + 1] and py < y.ptr[outer + 1]) {
                if (x.idx[px] == y.idx[py]) {
                    ...
                    if (i_o != j_o) {
                        numeric.set(&o.data[o._index(j_o, i_o)], numeric.conj(x.data[px]));
                    }
                    ...
                    if (i_o != j_o) {
                        if (if (comptime meta.uploOf(Y) == .upper) j_o <= i_o else j_o >= i_o) {
        ...
        while (idx_x_outer < x.rows) : (idx_x_outer += 1) {
            ...
            while (px < x.ptr[idx_x_outer + 1]) : (px += 1) {
                ...
                if (if (comptime meta.uploOf(Y) == .upper) i_o <= j_o else i_o >= j_o) {
                    if (i_o == j_o and comptime meta.diagOf(Y) == .unit) {
                        op_(&o.data[o._index(i_o, j_o)], x.data[px], numeric.one(meta.Numeric(Y)));
    ...
    while (idx < o.rows) : (idx += 1) {
        if (utils.searchSparse(x, idx, idx) == null and utils.searchSparse(y, idx, idx) == null) {
            if (comptime meta.diagOf(Y) == .unit) {
                if (comptime op_ == numeric.add_)
                    o.data[o._index(idx, idx)] = numeric.one(meta.Numeric(O))
                else
                    o.data[o._index(idx, idx)] = numeric.neg(numeric.one(meta.Numeric(O)));
            }
        }
    }
}
