...
/// * `fn zmlAsin_(*O, X) void`: Computes the arcsine of `x` and stores it in
///   `o`.
///
/// If neither `O` nor `X` implement the required `zmlAsin_` method, the function
...
pub inline fn asin_(o: anytype, x: anytype) void {
    ...
    if (comptime types.isCustomType(O)) {
        if (comptime types.isCustomType(X)) { // O and X both custom
            if (comptime types.anyHasMethod(&.{ O, X }, "zmlAsin_", fn (*O, X) void, &.{ *O, X })) |Impl|
                return Impl.zmlAsin_(o, x);
        } else { // only O custom
            if (comptime types.hasMethod(O, "zmlAsin_", fn (*O, X) void, &.{ *O, X }))
                return O.zmlAsin_(o, x);
        }
    } else if (comptime types.isCustomType(X)) { // only X custom
        if (comptime types.hasMethod(X, "zmlAsin_", fn (*O, X) void, &.{ *O, X }))
            return X.zmlAsin_(o, x);
    }
