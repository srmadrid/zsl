        ...
        while (outer < x.size) : (outer += 1) {
            ...
            while (px < x.ptr[outer + 1] and py < y.ptr[outer + 1]) {
                if (x.idx[px] == y.idx[py]) {
                    ...
                    if (i_o != j_o) {
                        if (if (comptime types.uploOf(Y) == .upper) j_o <= i_o else j_o >= i_o) {
    ...
    while (idx < o.rows) : (idx += 1) {
        if (utils.searchSparse(x, idx, idx) == null and utils.searchSparse(y, idx, idx) == null) {
            if (comptime types.diagOf(Y) == .unit) {
                if (comptime op_ == numeric.add_)
                    o.data[o._index(idx, idx)] = numeric.one(types.Numeric(O))
                else
                    o.data[o._index(idx, idx)] = numeric.neg(numeric.one(types.Numeric(O)));
            }
        }
    }
}
