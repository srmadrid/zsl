        ...
        while (outer < if (comptime meta.layoutOf(X) == .col_major) x.cols else x.rows) : (outer += 1) {
            ...
            while (px < x.ptr[outer + 1] and py < y.ptr[outer + 1]) {
                if (x.idx[px] == y.idx[py]) {
                    ...
                    if (x.idx[px] == outer and comptime meta.diagOf(Y) == .unit)
            ...
            if (comptime meta.diagOf(Y) == .unit) {
                if (outer < int.min(o.rows, o.cols)) {
                    if (utils.searchSparse(x, outer, outer) == null) {
                        if (comptime op_ == numeric.add_)
                            numeric.set(&o.data[o._index(outer, outer)], numeric.one(meta.Numeric(Y)))
                        else
                            numeric.set(&o.data[o._index(outer, outer)], numeric.neg(numeric.one(meta.Numeric(Y))));
                    }
                }
            }
        }
        ...
        while (idx_x_outer < if (comptime meta.layoutOf(X) == .col_major) x.cols else x.rows) : (idx_x_outer += 1) {
            ...
            while (px < x.ptr[idx_x_outer + 1]) : (px += 1) {
                ...
                if (i_o == j_o and comptime meta.diagOf(Y) == .unit) {
                    op_(&o.data[o._index(i_o, j_o)], x.data[px], numeric.one(meta.Numeric(Y)));
                } else if (if (comptime meta.uploOf(Y) == .upper) i_o <= j_o else i_o >= j_o) {
        ...
        if (comptime meta.diagOf(Y) == .unit) {
            ...
            while (idx < int.min(o.rows, o.cols)) : (idx += 1) {
                if (utils.searchSparse(x, idx, idx) == null) {
                    if (comptime op_ == numeric.add_)
                        numeric.set(&o.data[o._index(idx, idx)], numeric.one(meta.Numeric(Y)))
                    else
                        numeric.set(&o.data[o._index(idx, idx)], numeric.neg(numeric.one(meta.Numeric(Y))));
                }
            }
        }
    }
