...
pub fn apply2_(o: anytype, x: anytype, y: anytype, comptime op_: anytype) void {
    ...
    if (comptime types.layoutOf(O) == .col_major) {
        ...
        while (j < o.cols) : (j += 1) {
            if (comptime types.uploOf(Y) == .upper) {
                ...
                op_(&o.data[o._index(j, j)], numeric.zero(types.Numeric(X)), ty);
        ...
        while (i < o.rows) : (i += 1) {
            if (comptime types.uploOf(Y) == .lower) {
                ...
                op_(&o.data[o._index(i, i)], numeric.zero(types.Numeric(X)), ty);
    ...
    while (k < x.rows) : (k += 1) {
        ...
        const ty = if (i_o == j_o)
            (if (comptime types.diagOf(Y) == .unit) numeric.one(types.Numeric(Y)) else y.data[y._index(i_o, i_o)])
        else if (i_o < j_o)
            (if (comptime types.uploOf(Y) == .upper) y.data[y._index(i_o, j_o)] else numeric.zero(types.Numeric(Y)))
        else
            (if (comptime types.uploOf(Y) == .lower) y.data[y._index(i_o, j_o)] else numeric.zero(types.Numeric(Y)));
