...
pub fn apply2_(o: anytype, x: anytype, y: anytype, comptime op_: anytype) void {
    ...
    if (comptime meta.layoutOf(X) == meta.layoutOf(Y)) {
                    ...
                    if (i_o != j_o) {
                        if (utils.searchSparse(x, j_o, i_o) == null) {
                            if (comptime op_ == numeric.add_)
                                numeric.set(&o.data[o._index(j_o, i_o)], y.data[py])
                            else
                                numeric.set(&o.data[o._index(j_o, i_o)], numeric.neg(y.data[py]));
                        }
                    }
        ...
        while (idx_x_outer < x.cols) : (idx_x_outer += 1) {
