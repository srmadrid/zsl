...
/// ## Signature
/// ```zig
/// numeric.erf_(o: *O, x: X, ctx: anytype) !void
/// ```
///
/// ## Arguments
...
/// * `fn zmlErf_(*O, X, anytype) !void`: Computes the error function of `x` and
...
pub inline fn erf_(o: anytype, x: anytype, ctx: anytype) !void {
    ...
    if (comptime types.isCustomType(O)) {
        if (comptime types.isCustomType(X)) { // O and X both custom
            ...
            if (comptime Impl != null) {
                return Impl.?.zmlErf_(o, x, ctx);
            } else {
                var erf = try numeric.erf(x, ctx);
                defer numeric.deinit(&erf, ctx);
            ...
            if (comptime types.hasMethod(O, "zmlErf_", fn (*O, X, anytype) anyerror!void, &.{ *O, X, @TypeOf(ctx) })) {
                return O.zmlErf_(o, x, ctx);
            } else {
                var erf = try numeric.erf(x, ctx);
                defer numeric.deinit(&erf, ctx);
        ...
        if (comptime types.hasMethod(X, "zmlErf_", fn (*O, X, anytype) anyerror!void, &.{ *O, X, @TypeOf(ctx) })) {
            return X.zmlErf_(o, x, ctx);
        } else {
            var erf = try numeric.erf(x, ctx);
            defer numeric.deinit(&erf, ctx);
    ...
    switch (comptime types.numericType(O)) {
        .bool, .int, .float, .dyadic, .cfloat => switch (comptime types.numericType(X)) {
            ...
            .dyadic => @compileError("zml.numeric.erf_: not implemented for " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " yet."),
            .cfloat => @compileError("zml.numeric.erf_: not implemented for " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " yet."),
            .integer => @compileError("zml.numeric.erf_: not implemented for " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " yet."),
            .rational => @compileError("zml.numeric.erf_: not implemented for " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " yet."),
            .real => @compileError("zml.numeric.erf_: not implemented for " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " yet."),
            .complex => @compileError("zml.numeric.erf_: not implemented for " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " yet."),
            .custom => unreachable,
        },
