...
/// ## Signature
/// ```zig
/// numeric.atanh_(o: *O, x: X, ctx: anytype) !void
/// ```
///
/// ## Arguments
...
/// * `fn zmlAtanh_(*O, X, anytype) !void`: Computes the hyperbolic arctangent
...
///   context.
///
/// If neither `O` nor `X` implement the required `zmlAtanh_` method, the function
/// will fall back to using `numeric.set` with the result of `numeric.atanh`,
...
pub inline fn atanh_(o: anytype, x: anytype, ctx: anytype) !void {
    ...
    if (comptime types.isCustomType(O)) {
        if (comptime types.isCustomType(X)) { // O and X both custom
            ...
            if (comptime Impl != null) {
                return Impl.?.zmlAtanh_(o, x, ctx);
            } else {
                var atanh = try numeric.atanh(x, ctx);
                defer numeric.deinit(&atanh, ctx);
            ...
            if (comptime types.hasMethod(O, "zmlAtanh_", fn (*O, X, anytype) anyerror!void, &.{ *O, X, @TypeOf(ctx) })) {
                return O.zmlAtanh_(o, x, ctx);
            } else {
                var atanh = try numeric.atanh(x, ctx);
                defer numeric.deinit(&atanh, ctx);
        ...
        if (comptime types.hasMethod(X, "zmlAtanh_", fn (*O, X, anytype) anyerror!void, &.{ *O, X, @TypeOf(ctx) })) {
            return X.zmlAtanh_(o, x, ctx);
        } else {
            var atanh = try numeric.atanh(x, ctx);
            defer numeric.deinit(&atanh, ctx);
    ...
    switch (comptime types.numericType(O)) {
        .bool, .int, .float, .dyadic, .cfloat => switch (comptime types.numericType(X)) {
            ...
            .dyadic => @compileError("zml.numeric.atanh_: not implemented for " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " yet."),
            .cfloat => {
                comptime types.validateContext(@TypeOf(ctx), .{});
            ...
            .integer => @compileError("zml.numeric.atanh_: not implemented for " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " yet."),
            .rational => @compileError("zml.numeric.atanh_: not implemented for " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " yet."),
            .real => @compileError("zml.numeric.atanh_: not implemented for " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " yet."),
            .complex => @compileError("zml.numeric.atanh_: not implemented for " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " yet."),
            .custom => unreachable,
        },
