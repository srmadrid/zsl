...
pub fn Hypot(X: type, Y: type) type {
    ...
    switch (comptime types.numericType(X)) {
        .bool => switch (comptime types.numericType(Y)) {
            ...
            .int => @compileError("zml.numeric.hypot: not defined for " ++ @typeName(X) ++ " and " ++ @typeName(Y) ++ "."),
        ...
        .int => switch (comptime types.numericType(Y)) {
            .bool, .int => @compileError("zml.numeric.hypot: not defined for " ++ @typeName(X) ++ " and " ++ @typeName(Y) ++ "."),
            .rational => return rational.Hypot(X, Y),
            .float => return float.Hypot(X, Y),
            .dyadic => return dyadic.Hypot(X, Y),
            .complex => return complex.Hypot(X, Y),
...
///
/// `numeric.Hypot(X, Y)`, `X` or `Y` must implement the required `zmlHypot` method.
/// The expected signatures and behavior of `zmlHypot` are as follows:
/// * `fn zmlHypot(X, Y) numeric.Hypot(X, Y)`: Returns the hypotenuse of `x` and `y`.
    ...
    if (comptime types.isCustomType(X)) {
        if (comptime types.isCustomType(Y)) { // X and Y both custom
            const Impl: type = comptime types.anyHasMethod(
                ...
                @compileError("zml.numeric.hypot: " ++ @typeName(R) ++ ", " ++ @typeName(X) ++ " or " ++ @typeName(Y) ++ " must implement `fn zmlHypot(" ++ @typeName(X) ++ ", " ++ @typeName(Y) ++ ") " ++ @typeName(R) ++ "`");
            ...
            const Impl: type = comptime types.anyHasMethod(
                ...
                @compileError("zml.numeric.hypot: " ++ @typeName(R) ++ " or " ++ @typeName(X) ++ " must implement `fn zmlHypot(" ++ @typeName(X) ++ ", " ++ @typeName(Y) ++ ") " ++ @typeName(R) ++ "`");
        ...
        const Impl: type = comptime types.anyHasMethod(
            ...
            @compileError("zml.numeric.hypot: " ++ @typeName(R) ++ " or " ++ @typeName(Y) ++ " must implement `fn zmlHypot(" ++ @typeName(X) ++ ", " ++ @typeName(Y) ++ ") " ++ @typeName(R) ++ "`");
