...
pub fn isNumeric(comptime T: type) bool {
    switch (comptime @typeInfo(T)) {
        ...
        .@"struct" => return @hasDecl(T, "zml_is_numeric") and T.zml_is_numeric,
        else => return false,
    }
}
...
pub fn isVector(comptime T: type) bool {
    switch (comptime @typeInfo(T)) {
        .@"struct" => return @hasDecl(T, "zml_is_vector") and T.zml_is_vector,
        else => return false,
    }
}
...
pub fn isDenseVector(comptime T: type) bool {
    switch (comptime @typeInfo(T)) {
        .@"struct" => return @hasDecl(T, "zml_is_vector") and T.zml_is_vector and @hasDecl(T, "zml_is_dense") and T.zml_is_dense,
        else => return false,
    }
}
...
pub fn isSparseVector(comptime T: type) bool {
    switch (comptime @typeInfo(T)) {
        .@"struct" => return @hasDecl(T, "zml_is_vector") and T.zml_is_vector and @hasDecl(T, "zml_is_sparse") and T.zml_is_sparse,
        else => return false,
    }
}
...
pub fn isCustomVector(comptime T: type) bool {
    switch (comptime @typeInfo(T)) {
        .@"struct" => return @hasDecl(T, "zml_is_vector") and T.zml_is_vector and @hasDecl(T, "zml_is_custom") and T.zml_is_custom,
        else => return false,
    }
}
...
pub fn isMatrix(comptime T: type) bool {
    switch (comptime @typeInfo(T)) {
        .@"struct" => return @hasDecl(T, "zml_is_matrix") and T.zml_is_matrix,
        else => return false,
    }
}
...
pub fn isSquareMatrix(comptime T: type) bool {
    switch (comptime @typeInfo(T)) {
        .@"struct" => return (@hasDecl(T, "zml_is_symmetric") and T.zml_is_symmetric) or (@hasDecl(T, "zml_is_hermitian") and T.zml_is_hermitian),
        else => return false,
    }
}
...
pub fn isGeneralMatrix(comptime T: type) bool {
    switch (comptime @typeInfo(T)) {
        .@"struct" => return @hasDecl(T, "zml_is_matrix") and T.zml_is_matrix and @hasDecl(T, "zml_is_general") and T.zml_is_general,
        else => return false,
    }
}
...
pub fn isGeneralDenseMatrix(comptime T: type) bool {
    switch (comptime @typeInfo(T)) {
        .@"struct" => @hasDecl(T, "zml_is_matrix") and T.zml_is_matrix and @hasDecl(T, "zml_is_general") and T.zml_is_general and @hasDecl(T, "zml_is_dense") and T.zml_is_dense,
        else => return false,
    }
}
...
pub fn isGeneralSparseMatrix(comptime T: type) bool {
    switch (comptime @typeInfo(T)) {
        .@"struct" => @hasDecl(T, "zml_is_matrix") and T.zml_is_matrix and @hasDecl(T, "zml_is_general") and T.zml_is_general and @hasDecl(T, "zml_is_sparse") and T.zml_is_sparse,
        else => return false,
    }
}
...
pub fn isSymmetricMatrix(comptime T: type) bool {
    switch (comptime @typeInfo(T)) {
        .@"struct" => return @hasDecl(T, "zml_is_matrix") and T.zml_is_matrix and @hasDecl(T, "zml_is_symmetric") and T.zml_is_symmetric,
        else => return false,
    }
}
...
pub fn isSymmetricDenseMatrix(comptime T: type) bool {
    switch (comptime @typeInfo(T)) {
        .@"struct" => @hasDecl(T, "zml_is_matrix") and T.zml_is_matrix and @hasDecl(T, "zml_is_symmetric") and T.zml_is_symmetric and @hasDecl(T, "zml_is_dense") and T.zml_is_dense,
        else => return false,
    }
}
...
pub fn isSymmetricSparseMatrix(comptime T: type) bool {
    switch (comptime @typeInfo(T)) {
        .@"struct" => @hasDecl(T, "zml_is_matrix") and T.zml_is_matrix and @hasDecl(T, "zml_is_symmetric") and T.zml_is_symmetric and @hasDecl(T, "zml_is_sparse") and T.zml_is_sparse,
        else => return false,
    }
}
...
pub fn isHermitianMatrix(comptime T: type) bool {
    switch (comptime @typeInfo(T)) {
        .@"struct" => return @hasDecl(T, "zml_is_matrix") and T.zml_is_matrix and @hasDecl(T, "zml_is_hermitian") and T.zml_is_hermitian,
        else => return false,
    }
}
...
pub fn isHermitianDenseMatrix(comptime T: type) bool {
    switch (comptime @typeInfo(T)) {
        .@"struct" => @hasDecl(T, "zml_is_matrix") and T.zml_is_matrix and @hasDecl(T, "zml_is_hermitian") and T.zml_is_hermitian and @hasDecl(T, "zml_is_dense") and T.zml_is_dense,
        else => return false,
    }
}
...
pub fn isHermitianSparseMatrix(comptime T: type) bool {
    switch (comptime @typeInfo(T)) {
        .@"struct" => @hasDecl(T, "zml_is_matrix") and T.zml_is_matrix and @hasDecl(T, "zml_is_hermitian") and T.zml_is_hermitian and @hasDecl(T, "zml_is_sparse") and T.zml_is_sparse,
        else => return false,
    }
}
...
pub fn isTriangularMatrix(comptime T: type) bool {
    switch (comptime @typeInfo(T)) {
        .@"struct" => return @hasDecl(T, "zml_is_matrix") and T.zml_is_matrix and @hasDecl(T, "zml_is_triangular") and T.zml_is_triangular,
        else => return false,
    }
}
...
pub fn isTriangularDenseMatrix(comptime T: type) bool {
    switch (comptime @typeInfo(T)) {
        .@"struct" => @hasDecl(T, "zml_is_matrix") and T.zml_is_matrix and @hasDecl(T, "zml_is_triangular") and T.zml_is_triangular and @hasDecl(T, "zml_is_dense") and T.zml_is_dense,
        else => return false,
    }
}
...
pub fn isTriangularSparseMatrix(comptime T: type) bool {
    switch (comptime @typeInfo(T)) {
        .@"struct" => @hasDecl(T, "zml_is_matrix") and T.zml_is_matrix and @hasDecl(T, "zml_is_triangular") and T.zml_is_triangular and @hasDecl(T, "zml_is_sparse") and T.zml_is_sparse,
        else => return false,
    }
}
...
pub fn isDiagonalMatrix(comptime T: type) bool {
    switch (comptime @typeInfo(T)) {
        .@"struct" => @hasDecl(T, "zml_is_matrix") and T.zml_is_matrix and @hasDecl(T, "zml_is_diagonal") and T.zml_is_diagonal,
        else => return false,
    }
}
...
pub fn isPermutationMatrix(comptime T: type) bool {
    switch (comptime @typeInfo(T)) {
        .@"struct" => @hasDecl(T, "zml_is_matrix") and T.zml_is_matrix and @hasDecl(T, "zml_is_permutation") and T.zml_is_permutation,
        else => return false,
    }
}
...
pub fn isDenseMatrix(comptime T: type) bool {
    switch (comptime @typeInfo(T)) {
        .@"struct" => return @hasDecl(T, "zml_is_matrix") and T.zml_is_matrix and @hasDecl(T, "zml_is_dense") and T.zml_is_dense,
        else => return false,
    }
}
...
pub fn isSparseMatrix(comptime T: type) bool {
    switch (comptime @typeInfo(T)) {
        .@"struct" => return @hasDecl(T, "zml_is_matrix") and T.zml_is_matrix and @hasDecl(T, "zml_is_sparse") and T.zml_is_sparse,
        else => return false,
    }
}
...
pub fn isCustomMatrix(comptime T: type) bool {
    switch (comptime @typeInfo(T)) {
        .@"struct" => return @hasDecl(T, "zml_is_matrix") and T.zml_is_matrix and @hasDecl(T, "zml_is_custom") and T.zml_is_custom,
        else => return false,
    }
}
...
pub fn isArray(comptime T: type) bool {
    switch (comptime @typeInfo(T)) {
        .@"struct" => return @hasDecl(T, "zml_is_array") and T.zml_is_array,
        else => return false,
    }
}
...
pub fn isDenseArray(comptime T: type) bool {
    switch (comptime @typeInfo(T)) {
        .@"struct" => return @hasDecl(T, "zml_is_array") and T.zml_is_array and @hasDecl(T, "zml_is_dense") and T.zml_is_dense,
        else => return false,
    }
}
...
pub fn isStridedArray(comptime T: type) bool {
    switch (comptime @typeInfo(T)) {
        .@"struct" => return @hasDecl(T, "zml_is_array") and T.zml_is_array and @hasDecl(T, "zml_is_strided") and T.zml_is_strided,
        else => return false,
    }
}
...
pub fn isSparseArray(comptime T: type) bool {
    switch (comptime @typeInfo(T)) {
        .@"struct" => return @hasDecl(T, "zml_is_array") and T.zml_is_array and @hasDecl(T, "zml_is_sparse") and T.zml_is_sparse,
        else => return false,
    }
}
...
pub fn isCustomArray(comptime T: type) bool {
    switch (comptime @typeInfo(T)) {
        .@"struct" => return @hasDecl(T, "zml_is_array") and T.zml_is_array and @hasDecl(T, "zml_is_custom") and T.zml_is_custom,
        else => return false,
    }
}
...
pub fn isIntegral(comptime N: type) bool {
    ...
    switch (comptime types.numericType(N)) {
        ...
        .int => return true,
        .float => return false,
        else => return @hasDecl(N, "zml_is_integral") and N.zml_is_integral,
    }
}
...
pub fn isSigned(comptime N: type) bool {
    ...
    switch (comptime types.numericType(N)) {
        ...
        .float => return true,
        else => return @hasDecl(N, "zml_is_signed") and N.zml_is_signed,
    }
}
...
pub fn isUnsigned(comptime N: type) bool {
    ...
    switch (comptime types.numericType(N)) {
        ...
        .float => return false,
        else => return @hasDecl(N, "zml_is_unsigned") and N.zml_is_unsigned,
    }
}
