            ...
            while (px < x.ptr[outer + 1] and py < y.ptr[outer + 1]) {
                if (x.idx[px] == y.idx[py]) {
                    ...
                    if (i_o == j_o and comptime meta.diagOf(X) == .unit) {
                        ...
                        if (if (comptime meta.uploOf(X) == .upper) i_o <= j_o else i_o >= j_o) {
                    ...
                    if (i_o != j_o) {
                        if (if (comptime meta.uploOf(X) == .upper) j_o <= i_o else j_o >= i_o) {
    ...
    if (comptime meta.diagOf(X) == .unit) {
        ...
        while (idx < o.rows) : (idx += 1) {
            if (utils.searchSparse(x, idx, idx) == null and utils.searchSparse(y, idx, idx) == null) {
                numeric.set(&o.data[o._index(idx, idx)], numeric.one(meta.Numeric(X)));
            }
        }
    }
}
