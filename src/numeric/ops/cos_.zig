...
/// ## Signature
/// ```zig
/// numeric.cos_(o: *O, x: X, ctx: anytype) !void
/// ```
///
/// ## Arguments
...
/// * `fn zmlCos_(*O, X, anytype) !void`: Computes the cosine of `x` and stores
...
pub inline fn cos_(o: anytype, x: anytype, ctx: anytype) !void {
    ...
    if (comptime types.isCustomType(O)) {
        if (comptime types.isCustomType(X)) { // O and X both custom
            ...
            if (comptime Impl != null) {
                return Impl.?.zmlCos_(o, x, ctx);
            } else {
                var cos = try numeric.cos(x, ctx);
                defer numeric.deinit(&cos, ctx);
            ...
            if (comptime types.hasMethod(O, "zmlCos_", fn (*O, X, anytype) anyerror!void, &.{ *O, X, @TypeOf(ctx) })) {
                return O.zmlCos_(o, x, ctx);
            } else {
                var cos = try numeric.cos(x, ctx);
                defer numeric.deinit(&cos, ctx);
        ...
        if (comptime types.hasMethod(X, "zmlCos_", fn (*O, X, anytype) anyerror!void, &.{ *O, X, @TypeOf(ctx) })) {
            return X.zmlCos_(o, x, ctx);
        } else {
            var cos = try numeric.cos(x, ctx);
            defer numeric.deinit(&cos, ctx);
    ...
    switch (comptime types.numericType(O)) {
        .bool, .int, .float, .dyadic, .cfloat => switch (comptime types.numericType(X)) {
            ...
            .dyadic => @compileError("zml.numeric.cos_: not implemented for " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " yet."),
            .cfloat => {
                comptime types.validateContext(@TypeOf(ctx), .{});
            ...
            .integer => @compileError("zml.numeric.cos_: not implemented for " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " yet."),
            .rational => @compileError("zml.numeric.cos_: not implemented for " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " yet."),
            .real => @compileError("zml.numeric.cos_: not implemented for " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " yet."),
            .complex => @compileError("zml.numeric.cos_: not implemented for " ++ @typeName(O) ++ " and " ++ @typeName(X) ++ " yet."),
            .custom => unreachable,
        },
